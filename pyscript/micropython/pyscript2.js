...
const main = function () {
    ...
    **************************************************************************/
    ...
    const logger = function () {
        /*
        Really simple logging. Emoji ðŸ”¬ðŸðŸ“œ highlights PyScript app logs. ;-)
        */
        return Function.prototype.bind.call(console.log, console, "ðŸ”¬ðŸðŸ“œ ", ...arguments);
    }();
    logger("Starting PyScript. ðŸ‘‹ðŸ”¬ðŸðŸ“œ");
    ...
    // "Loading PyScript...".
    const defaultSplash = '<div style="position:fixed;width:100%;height:100%;top:0;left:0;right:0;bottom:0;background-color:rgba(0,0,0,0.5);z-index:9999;"><div style="position:absolute;top:50%;left:50%;color:white;">Loading PyScript... ðŸ”¬ðŸ</div></div>';
    ...
    **************************************************************************/
    ...
    const pyScriptTag = function () {
        ...
        function registerScript(e) {
            ...
            */
            ...
            logger("Registered script. ðŸ“„", script);
            scripts.push(script);
            if (script.code) {
                ...
                fetch(script.src).then(function (response) {
                    logger(`Fetched script from "${script.src}". ðŸ“¡`, response);
                    if (response.ok) {
                        response.text().then((data) => {
                            script.code = data;
                            logger("Updated script code. ðŸ“„", script);
                            const pyLoadedScript = new CustomEvent("py-script-loaded", { detail: script });
                            document.dispatchEvent(pyLoadedScript);
                        })
                    } else {
                        // Abort.             
                        throw `ðŸ’¥ Cannot load script from "${script.src}"`;
                    }
                });
            } else {
                // Warn that a script has no source code either inline or via
                // the src attribute.
                logger("Script has no source code. â‰ï¸ðŸ˜•", script);
            }
        }
    ...
    function loadConfig() {
        ...
        */
        ...
        logger("Loaded configuration. âœ…", config);
    ...
    function registerPlugin(plugin) {
        ...
        method.
        */
        logger(`Registering plugin "${plugin.name}". ðŸ”Œ`);
    ...
    function startPlugin(plugin) {
        ...
        */
        logger(`Starting plugin "${plugin.name}". âš¡`);
        plugin.start?.(config);
        ...
        */
        ...
        if (config.files) {
            ...
            for (let path in config.files) {
                ...
                logger(`Fetching file "${path}" from: ${url} ðŸ“¡`);
                ...
                filePromise.then(response => {
                    if (response.ok) {
                        ...
                        // Abort.             
                        throw `ðŸ’¥ Cannot load file from "${url}"`;
                    }
                });
        ...
        Promise.all(pendingDownloads).then((values) => {
            filesLoaded = true;
            if (values) {
                logger(`All files downloaded, copying to filesystem. ðŸ“¥`);
            }
    ...
    function onFileFetched(e) {
        ...
        */
        logger(`Saving file "${e.detail.path}" to file system. ðŸ’¾`);
    ...
    function loadRuntime() {
        ...
        */
        if (!runtimes.hasOwnProperty(config.runtime)) {
            throw `ðŸ’¥ Unknown runtime: "${config.runtime}" (known runtimes: ${Object.keys(runtimes)})`;
        }
        ...
        runtimeElement.onload = function (e) {
            logger(`Runtime "${config.runtime}" loaded. ðŸ‘`);
        ...
        logger(`Loading runtime "${config.runtime}". ðŸš€`)
    ...
    function finished() {
        ...
        */
        if (runtimeReady && filesLoaded) {
            logger(`PyScript finished setup. ðŸ`);
